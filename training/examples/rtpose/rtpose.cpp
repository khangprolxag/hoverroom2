...
void* getFrameFromCam(void *i) {
    ...
    if (FLAGS_video.empty()) {
        //http://192.168.1.68:8080/?action=stream
...
void* displayFrame(void *i) { //single thread
    ...
    while(1) {
        ...
        if (FLAGS_write_frames.empty()) {
            snprintf(tmp_str, 256, "%4f fps", FPS);
        } else {
            snprintf(tmp_str, 256, "%4f s/gpu", FLAGS_num_gpu*1.0/FPS);
        }
        if (!FLAGS_no_text) {
        ...
        if (!FLAGS_video.empty() && FLAGS_write_frames.empty()) {
            snprintf(tmp_str, 256, "Khung hinh %6d", global.uistate.current_frame);
    ...
    if (!FLAGS_image_dir.empty()) {
        ...
        for ( boost::filesystem::directory_iterator itr( folderName ); itr != end_itr; ++itr ) {
            if ( boost::filesystem::is_directory(itr->status()) ) {
                ...
                LOG(ERROR) << filename;
                global.image_list.push_back( filename );
            }
        }
